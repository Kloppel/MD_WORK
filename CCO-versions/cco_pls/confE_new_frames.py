...
def conf_energy_computation(pdb_structure_cco, modelling_folder, jobname, cavity_parameter=None, apbs=False, small=None,
                            glu_deprot=False, asp_deprot=False, propionic_patch = None):
...
"""
    ...
    #     pdb_structure_og = '/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/final_models/combo_prdh.pdb'
    #     test_folder = '/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/prdh/'
    #
...
#     ###################################################################################################
#
#     run_script_file = '/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/Pr_prdh/run_script.sh'
#     run_script = """#!/bin/tcsh
#
...
# """
#
#     frames = np.arange(0,201,10)
#     tests = ['']
#     for test in tests:
#         for frame in frames:
#             pdb_structure_og = '/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/MD/Pr_prdh/frames_voda/frame%i.pdb' % (frame)
#             run_folder = '/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/Pr_prdh/f%i/' % (frame)
...
#                 frame_script = """
#
# set JOBNAME=model_f%i       """ % frame + """
# set SDIR=/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/Pr_prdh/f%i/model_f%i_w/apbs/  """ % (frame, frame) + """
# set JDIR=/public/scratch/jdragelj/apbs/$JOBNAME
#
#
# mkdir -p $JDIR
# cp $SDIR/* $JDIR
...
# cd $JDIR
#
# /scratch/scratch/tmeyer/CHARMM_NAMD/cavityfinder < cavityFinder.in > cavityFinder.out
# /scratch/scratch/tmeyer/CHARMM_NAMD/apbs apbs.in > apbs.out
# /scratch/scratch/tmeyer/CHARMM_NAMD/coulomb -e model_f%i_w.pqr > coulomb.out   """ % (frame) + """
#
# cd ..
#
...
#
# """
#                 run_script += frame_script
#
#
#                 conf_energy_computation(pdb_structure_w, modelling_folder, jobname, cavity_parameter=0.9, apbs=True,
#                                         small=string, glu_deprot=False, asp_deprot=True, propionic_patch = 'PRDH')
#             else:
#                 continue
#     f = open(run_script_file, 'w')
#     f.write(run_script)
#     f.close()
...
################################################################################################################################################################################################################################
    ...
    #
    print 'crystal'
    modelling_folder = '/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/conf_ene_Pr_prd_a3/minimized/crystal/e80_05_test/crystal_min_w/'
    run_folder = modelling_folder + 'apbs/'
    ...
    print "**********Conf energy: %.2f KJ/mol (apbs %.2f, elec %.2f)" % (
    conf_energy_closed_w, apbs_energy, charmm_energy['ELEC'][0])

    print 'open sb model'
    modelling_folder = '/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/conf_ene_Pr_prd_a3/minimized/model/e80_05_test/model_min_w/'
    run_folder = modelling_folder + 'apbs/'
    apbs_energy, coulomb_energy = kbp2.apbs_manager.read_result(run_folder, epsilon=4.0)
    charmm_energy = kbp2.charmm.get_charmm_energy(modelling_folder)
    ...
    print 'new model open sb open hb'
    modelling_folder = '/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/prdh/model_min_w/'
    run_folder = modelling_folder + 'apbs/'
    apbs_energy, coulomb_energy = kbp2.apbs_manager.read_result(run_folder, epsilon=4.0)
    charmm_energy = kbp2.charmm.get_charmm_energy(modelling_folder)
    conf_energy_open_w = apbs_energy + charmm_energy['ELEC'][0]
    ...
    print 'new model open sb open hb'
    modelling_folder = '/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/Pr_prdh2/model_min_w/'
    run_folder = modelling_folder + 'apbs/'
    apbs_energy, coulomb_energy = kbp2.apbs_manager.read_result(run_folder, epsilon=4.0)
    charmm_energy = kbp2.charmm.get_charmm_energy(modelling_folder)
    conf_energy_open_w = apbs_energy + charmm_energy['ELEC'][0]
    ...
    for ft in frames_tests:
        ...
        energies = []
        for frame in frames:
            modelling_folder = '/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/Pr_prdh/f%i/model_f%i_w/' % (frame, frame)
            run_folder = modelling_folder + 'apbs/'
            apbs_energy, coulomb_energy = kbp2.apbs_manager.read_result(run_folder, epsilon=4.0)
            charmm_energy = get_charmm_energy_many(modelling_folder, k=1)
            conf_energy_open_w = apbs_energy + charmm_energy['ELEC'][0]
        ...
        energies = []
        for frame in frames:
            modelling_folder = '/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/Pr_prdh2/f%i/model_f%i_w/' % (frame, frame)
            run_folder = modelling_folder + 'apbs/'
            apbs_energy, coulomb_energy = kbp2.apbs_manager.read_result(run_folder, epsilon=4.0)
            charmm_energy = get_charmm_energy_many(modelling_folder, k=1)
            conf_energy_open_w = apbs_energy + charmm_energy['ELEC'][0]
        ...
        ax1.plot(frame_scale, avgM, lw=1.0, linestyle='--', color='blue')
        ...
        plt.ylim(0,700)
        plt.savefig('/home/users/j/jdragelj/Desktop/SHARED_ConfE_25ns.png')

        # plt.show()

    # energies = []
        # for frame in frames:
        #     modelling_folder = '/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/PF_PRDa-/f%i/model_f%i_w/' % (
        #     frame, frame)
        #     run_folder = modelling_folder + 'apbs/'
        #     apbs_energy, coulomb_energy = kbp2.apbs_manager.read_result(run_folder, epsilon=4.0)
        #     charmm_energy = get_charmm_energy_many(modelling_folder, k=1)
        ...
        # plt.rcParams.update({'font.size': 16})
        # plt.ylim(0,700)
        # # plt.savefig('/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/ConfE_25ns.png' )
        # plt.savefig('/mnt/fu-scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/new_models/PRD_H_PRA_a3/conf_E/SB_HB_D/MD/ConfE_PRDa_25ns.png' )
        #
        #
        #
