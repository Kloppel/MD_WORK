...
if __name__ == '__main__':
    ...
    residues = ['PRD-3_EHEM', 'PRA-1_GHEM', 'PRD-3_GHEM', \
                'PRA-1_EHEM',
                'ARG-481_ACHA', 'ARG-482_ACHA',\
                    'GLU-286_ACHA', \
    ...
    #
    # # chunk2 = np.arange(200, 301, 1)
    # chunk2 = np.arange(0, 361, 2)
    # frame_range = chunk2
    ...
    # chunk2 = np.arange(0, 361, 2)
    # frame_range = chunk2
    # workfolder = titration_folder + 'md_PF_prah_asp_dep/'
    # if not os.path.exists(workfolder):
    #     os.mkdir(workfolder)
    # frames_folder = '/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/PRA_a3/md_PF_prah_asp_dep/%s/' % frames_suffix
    # state = 'PF'
    # md_name = 'asp_PF_trans'
    # titrate_structures_dolly(workfolder, frames_folder, frame_range, md_name, state, residues_tt = residues, cavities=[0.9, 0.2, 0.0],
    #                          membrane_charge = False, kbp2_eval_titration = True, glu_asp_lys_patch=[False, False, True])
    # ######################################################################################################################################################
    ...
    chunk2 = np.arange(0, 200, 2)
    frame_range = chunk2
    workfolder = titration_folder + 'md_PF_prah_asph/'
    if not os.path.exists(workfolder):
        os.mkdir(workfolder)
    frames_folder = '/scratch/scratch/jdragelj/projects/cco_pls/prd_pra/open_sb_2GSM/PRA_a3/md_PF_prah_asph/%s/' % frames_suffix
    state = 'PF'
    md_name = 'PF_asph_prah'
    titrate_structures_dolly(workfolder, frames_folder, frame_range, md_name, state, residues_tt = residues, cavities=[0.9, 0.2, 0.0],
                             membrane_charge = False, kbp2_eval_titration = True, glu_asp_lys_patch=[False, False, True])
    ######################################################################################################################################################